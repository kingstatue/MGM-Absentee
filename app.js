...
function lockAppScroll(locked) {
    try {
        const anyOverlay = document.querySelector(
            '.modal-overlay.active, .history-drawer.active, .toast-overlay.active, .dept-login-overlay.active, #slotConflictModalDialog, #deleteConfirmModalDialog, #alertModalDialog, #missingComboModalDialog, #bulkGeneratorModal.active, #registerScannerModal.active'
        );
...
function historyYearSortRank(yearVal) {
    const y = (typeof normalizeYearCode === 'function') ? normalizeYearCode(yearVal) : String(yearVal || '');
    if (y === '1') return 1;
    if (y === '2') return 2;
    if (y === '3') return 3;
    return 9;
...
function historyGroupKey(item) {
    return historyYearSortRank(item.year) + '|' +
        ((typeof normalizeYearCode === 'function') ? normalizeYearCode(item.year) : String(item.year || '')) + '|' +
        normalizeSectionCode(item.section || '');
}

function historyGroupLabel(item) {
    return (item.year || 'Year') + ' — Sec ' + (item.section || 'A');
}
...
function populateHistorySectionFilter() {
    ...
    const isFirstYear = selYear === 'First Year';

    const options = [
        { val: 'ALL', label: 'All Sections' },
        { val: 'A', label: 'Section A' },
        { val: 'B', label: 'Section B' },
        { val: 'C', label: isFirstYear ? 'Section C (AIML)' : 'Section C' },
...
/** Subject options for history drawer only — built from current year/section/date slice. */
function populateHistorySubjectFilter() {
    ...
    let base = (currentHistoryTabMode === 'ALL')
        ? readAllHistory().filter(item => (item.stream || 'BCA') === currentDept)
        : getTodayEntries();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const sectionFilterEl = document.getElementById('allHistorySectionFilter');
    const subjectFilterEl = document.getElementById('allHistorySubjectFilter');
    ...
    if (sectionFilterEl) sectionFilterEl.addEventListener('change', () => {
        try { populateHistorySubjectFilter(); } catch (e) {}
        renderHistoryList();
    });
    if (subjectFilterEl) subjectFilterEl.addEventListener('change', () => renderHistoryList());
    if (dateFilterEl) dateFilterEl.addEventListener('change', () => {
        try { populateHistorySubjectFilter(); } catch (e) {}
        renderHistoryList();
    });
    if (clearFilterBtn) clearFilterBtn.addEventListener('click', () => {
        if (yearFilterEl) yearFilterEl.value = 'ALL';
        if (sectionFilterEl) sectionFilterEl.value = 'ALL';
...
(function checkAppCacheVersion() {
    ...
    const APP_VER = 'v29.34-space-hdr';
...
function shortageClassLabel(yearStr, sectionStr) {
    const roman = { 'First Year': 'I', 'Second Year': 'II', 'Third Year': 'III' };
    const r = roman[yearStr] || yearStr || '';
    const sec = !sectionStr || sectionStr === 'ALL' ? 'Combined' : sectionStr;
    return r + ' BCA ' + sec;
}
...
function buildOfficialAttendanceSheetTable(meta, shortageList) {
    ...
    const classLabel = escapeHTML(shortageClassLabel(meta.yearStr, meta.sectionStr));
...
function downloadOfficialShortageExcel(meta, shortageList) {
    ...
    const classPart = shortageClassLabel(meta.yearStr, meta.sectionStr).replace(/\s+/g, '_');
